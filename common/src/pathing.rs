...
impl CrewNav {
    ...
    fn step(&mut self, nav_mesh: &NavMesh) -> Poll<Cell> {
        ...
        let arrived = match &mut self.current_location {
            NavLocation::Line(line, x) => {
                ...
                *x = x.move_toward(target_x, 1.0 / 64.0);
                *x == target_x
            }
            NavLocation::Square(square, (x, y)) => {
                let (target_x, target_y) = square.coords_of(current_goal);
                *x = x.move_toward(target_x, 1.0 / 64.0);
                *y = y.move_toward(target_y, 1.0 / 64.0);
    ...
    fn current_cell(&self) -> Cell {
        match self.current_location {
            ...
            NavLocation::Square(square, (x, y)) => square.0[round_to_usize(y)][round_to_usize(x)],
        }
    }
}
...
impl SquareSection {
    pub fn coords_of(&self, cell: Cell) -> (f32, f32) {
        self.0
            .iter()
            .enumerate()
